...
h = [handles.pushbutton1 handles.pushbutton2 handles.pushbutton3 handles.edit2 handles.edit3 handles.text10 handles.text5];
...
function togglebutton1_Callback(~, ~, handles)

if get(handles.togglebutton1,'Value') == 0
    set(handles.togglebutton1,'String','difference')
else
    set(handles.togglebutton1,'String','transect')
end
...
clear T_hist E_hist acq timestamp elapsedSec timeSec

persistent kiac kibc kicc kidc nw savename videofile writerObj T_hist E_hist acq timestamp elapsedSec timeSec
...
for i=good_data(good_data>=fi & good_data<=fl)
    ...
    plot_axes('Elapsed Time (s)', 'Temperature (K)', strcat(filename(1:end-5),'  :',num2str(timevector(1,4)),':',num2str(timevector(1,5)),':',num2str(timevector(1,6)),{'  '},num2str(progress),'%'),elapsedSec,T_hist, plot_type, dy, dx, sa, microns);
    ...
    if get(handles.togglebutton1,'Value') == 0
        ...
        axes(handles.axes4)
        plot(microns,T(dx,(1:length(T))));
        hold on
        plot(microns,T(1:length(T),dy));
        hold off
...
if get(handles.pushbutton4,'Value') == 0
    set(handles.text15,'BackgroundColor',[1.0 0.6 0.784]);
    ...
    set(handles.text15,'BackgroundColor',[0 0.8 0]);
