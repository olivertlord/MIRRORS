...
% Last Modified by GUIDE v2.5 24-Jun-2015 09:43:25
...
fcn = makeConstrainToRectFcn('imrect',[10 372],[10 245]);
...
fl = eval(get(handles.edit3,'string'));
fg = find(getappdata(0,'good_data')>0,1);
...
for i=fi:fl
    ...
    %is larger than 2*noise and which are not close to saturation
        ...
        if fcnt2 == 2;
            first = (T - min(T(:)))/(max(T(:)) - min(T(:)));
            fsuid = (suid - min(suid(:)))/(max(suid(:)) - min(suid(:)));
        end;
        
        if fcnt2 >= 2;
            last = (T - min(T(:)))/(max(T(:)) - min(T(:)));
            lsuid = (suid - min(suid(:)))/(max(suid(:)) - min(suid(:)));
        end;
        
        [Tmax,Emax,T,error,Emissivity,umax,slope_max,intercept_max,dx,dy,suid,uia,T1]=mapper(x,y,w,skiac,skibc,skicc,skidc,nw,byd,bxd,cyd,cxd,dyd,dxd,d,a,c,b,fcnt);
        %calls mapper function to calculate temperature map
        
        if fcnt2 >= 2;
            current = (T - min(T(:)))/(max(T(:)) - min(T(:)));
            csuid = (suid - min(suid(:)))/(max(suid(:)) - min(suid(:)));
            difT_c = abs(current-first);
            difT_i = abs(current-last).*csuid;
            metric(fcnt2) = mean(difT_i(:));
            difT_plot=difT_i+difT_plot;
        end;
        chi2_plot(fcnt2)=metric(fcnt2);
        ...
        autoresult = [seq',timevalue',elapsedSecNorm',Tmaximum',Emaximum',chi2_plot'];
        ...
        title_string=strcat(num2str(S(1,4)),':',num2str(S(1,5)),':',num2str(S(1,6)),{'  '},num2str(progress),'%');
        title(title_string,'FontSize',18,'FontWeight','bold');
        xl = xlim;
...
%         %through the hottest pixel
        ...
        ax3 = subplot(2,3,5);
        chi2_plot(chi2_plot==0) = NaN;
        plot(ax3,time_plot,chi2_plot,'--rs','LineWidth',1,...
            'MarkerEdgeColor','b',...
            'MarkerFaceColor','b',...
            'MarkerSize',10);
        xlabel('Elapsed Time (s)','FontSize',16);
        ...
        if fcnt2 > 1 && Clim_min > min(difT_i(:));
            Clim_min = min(difT_i(:));
        end;
        if fcnt2 > 1 && Clim_max < max(difT_i(:));
            Clim_max = max(difT_i(:));
        end;
        Clim = [Clim_min Clim_max];
        originalSize = get(gca, 'Position');
        colormap jet;
        imagesc(xm,ym,difT_i,Clim);
        xlabel('Distance (microns)','FontSize',16)
