function [Tmax,Emax,T,error,Emissivity,umax,slope_max,intercept_max,dx,dy,suib]=mapper(x,y,hw,skiac,skibc,skidc,nw,byd,bxd,dyd,dxd,d,a,b)
...
Icut2 = ((max(suib(:))-min(suib(:)))/3)+min(suib(:));
%make cutoff at 10th intensity percentile
...
for m=11:k-10
    for n=11:k-10
        if  suib(m,n) > Icut2*2 && T(m,n)>Tmax & isempty(find(isnan(T(m-10:m+10,n-10:n+10))));
            Tmax=T(m,n);
            dx = m;
            dy = n;
            Emax=error(dx,dy);
            ...
            umax=[Ja(dx,dy) Jb(dx,dy) Jd(dx,dy)]';
        end
    end
end
