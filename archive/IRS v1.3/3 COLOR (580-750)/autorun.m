...
savename=strcat(savename,'summary_(3color, ',num2str(x),',',num2str(y),',',num2str(w),',',num2str(fi),',',num2str(fl),')');
...
[byd,bxd,dyd,dxd]= correlate(kia,kib,kid);
...
nw=[(14384000/752.97);(14384000/578.61);(14384000/670.08)];
nw=[ones(3,1) nw];
...
for i=fi:fl
    ...
    if maxA*0.8 > noise*2 && maxB*0.8 > noise*2 && maxD*0.8 > noise*2 && maxA < 64000 && maxB < 64000 && maxD <64000;
    ...
    %is larger than 2*noise and which are not close to saturation
        ...
        if fcnt1 == 1;
            [byd,bxd,dyd,dxd]= correlate(a,b,d);
        end;
        %determines offsets based on first unknown file
        
        [Tmax,Emax,T,error,Emissivity,umax,slope_max,intercept_max,dx,dy,suib]=mapper(x,y,w,skiac,skibc,skidc,nw,byd,bxd,dyd,dxd,d,a,b);
        ...
        %stores figure for making .mov at end of program
    end
end
