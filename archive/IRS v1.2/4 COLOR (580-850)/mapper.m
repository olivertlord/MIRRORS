function [Tmax,Emax,T,error,Emissivity,umax,slope_max,intercept_max,dx,dy,suib]=mapper(x,y,hw,skiac,skibc,skicc,skidc,nw,byd,bxd,cyd,cxd,dyd,dxd,d,a,c,b)
...
for m=11:k-10
    for n=11:k-10
        if  suib(m,n) > ((Icut2*9.9)+min(suib(:))) && T(m,n)>Tmax & isempty(find(isnan(T(m-10:m+10,n-10:n+10))));
            Tmax=T(m,n);
            dx = m;
            dy = n;
            Emax=error(dx,dy);
