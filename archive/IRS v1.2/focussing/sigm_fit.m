...
[BETA,RESID,J] = nlinfit(x,y,f,initial_params(bool_vec==1));
stat.param=BETA';

% confidence interval of the parameters
stat.paramCI = nlparci(BETA,RESID,J);

% confidence interval of the estimation
[stat.ypred,delta] = nlpredci(f,x,BETA,RESID,J);
...
% if plot_flag==1 
%     x_vector=min(x):diff(min(x'))/100:max(x);
