function [Tmax,Emax,T,error,ui_average,Emissivity,dx,dy,umax,umean,slope,intercept]=dataprep(x,y,raw,hw,cutoff1,cutoff2,kiac,kibc,kidc,v,byd,bxd,cyd,cxd,dyd,dxd,fm)
        ...
        if (ui_average(m,n) < (cutoff1));
           ...
           error(m,n)=NaN;
            ...
            if T(m,n)>maxtemp;
               maxtemp=T(m,n);
               umax=u;
               umean=[(mean(uJa)),(mean(uJb)),(mean(uJd))];
               slope=wien(2);
               intercept=wien(1);
            end
        end
