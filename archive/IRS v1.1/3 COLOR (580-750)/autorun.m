...
calfilename = 'C:\MATLAB7\work\3 COLOR (580-750)\Calibration files\20.10.14 (dacb) Calibration\tc.tiff';
...
vp=[(repmat(14384000/752.97,81,1));(repmat(14384000/578.61,81,1));(repmat(14384000/670.08,81,1))];
v=[ones(243,1) vp];

vm=[(14384000/752.97);(14384000/578.61);(14384000/670.08)]
vmean=[ones(3,1) vm];
...
for i=m:n
    ...
    [Tmax,Emax,T,error,ui_average,Emissivity,dx,dy,umax,umean,slope,intercept]=dataprep(x,y,raw,w,cutoff1,cutoff2,kiac,kibc,kidc,v,byd,bxd,cyd,cxd,dyd,dxd,fm);
...
%title('Temperature vs. Error','FontSize',18,'FontWeight','bold');
%hold off
